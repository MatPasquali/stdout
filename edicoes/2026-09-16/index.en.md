# stdout · Edition of 2026-09-16

_The news, papers and projects moving the tech world right now: collected, ranked and written automatically._

## Highlights

### [Introducing System One Models and Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev)
`Hacker News`

Introducing System One Models and Jev

### [Show HN: An e-ink frame that hears birds and draws them as 1800s illustrations](https://github.com/arnegiacomo/fugleramme)
`Hacker News`

Show HN: An e-ink frame that hears birds and draws them as 1800s illustrations

### [ai-sucks-butt/ai-sucks-butt](https://github.com/ai-sucks-butt/ai-sucks-butt)
`GitHub`

If you think AI sucks, star the repo.

## Artificial Intelligence

### [A single firm is behind OpenAI, Anthropic, and Meta hacking scandals](https://www.effort.news/irregular)
`Hacker News`

A single firm is behind OpenAI, Anthropic, and Meta hacking scandals

### [ukisai/Swift-Qwen3.8-27b](https://huggingface.co/ukisai/Swift-Qwen3.8-27b)
`Hugging Face`

Modelo no Hugging Face para a tarefa 'image-text-to-text', com 305 likes e 2753 downloads.

### [The Download: AI doomers, whistleblowing agents, and de-aged livers](https://www.technologyreview.com/2026/09/15/1144141/the-download-ai-extinction-whistleblowing-agents-donated-livers/)
`MIT Tech Review`

This is today’s edition of The Download, our weekday newsletter that provides a daily dose of what’s going on in the world of technology. The AI industry has taken a doomer turn. What now? AI chiefs Dario Amodei, Sam Altman, Elon Musk, and Demis Hassabis are suddenly all in agreement: the latest generation of LLMs…

### [AI models need more data about biology, and OpenAI is paying to create it](https://www.technologyreview.com/2026/09/15/1144129/ai-models-need-more-data-about-biology-and-openai-is-paying-to-create-it/)
`MIT Tech Review`

Last year Ruxandra Teslo, a policy analyst who focuses on clinical trials, posted an idea for supercharging medical AI systems: Use data from failed biotech companies. By bidding at their bankruptcy proceedings, she proposed, it might be possible to obtain detailed regulatory filings, manufacturing strategies, and safety data, types of information usually considered trade secrets. She…

## Industry

### [25 years of mass surveillance is enough](https://www.schneier.com/blog/archives/2026/09/25-years-of-mass-surveillance-is-enough.html)
`Hacker News`

25 years of mass surveillance is enough

### [An update on Wayback Machine access](https://blog.archive.org/2026/09/15/an-update-on-wayback-machine-access/)
`Hacker News`

An update on Wayback Machine access

### [Dario, Please](https://pop.rdi.sh/dario-please/)
`Hacker News`

Dario, Please

### [Robots are waiting for a ChatGPT moment: Nvidia’s Les Karpas explains why at TechCrunch Disrupt 2026](https://techcrunch.com/2026/09/16/robots-are-waiting-for-a-chatgpt-moment-nvidias-les-karpas-explains-why-at-techcrunch-disrupt-2026/)
`TechCrunch`

The robotics industry is still waiting for their breakthrough into day-to-day life. Nvidia's Les Karpas has an answer as to why at TechCrunch Disrupt 2026. Register before September 25 to save up to $200 on your pass.

## Job Market

### [These Companies Want Interns This Summer (Sep 16)](https://dev.to/crackr/these-companies-want-interns-this-summer-sep-16-3a1o)
`dev.to · carreira`

ByteDance, Texas Instruments, Citadel, Zions Bank, SSOE Group, Citadel Securities are all hiring interns for Summer 2026. Here's what's open.


  
  
   ByteDance


1 open internships


  
  
  - Research Scientist Intern - Seed Responsible AI | San Jose, CA



  
  
   Texas Instruments


1 open internships


  
  
  - Smart Manufacturing and Automation Intern - SMA Quality & Test | Dallas, TX



  
  
   Citadel


1 open internships


  
  
  - Quantitative Researcher Post-Doctoral Intern | Gr

### [Evaluating ML-Based Hiring Tools: An Engineer's Checklist](https://dev.to/john_zacharia/evaluating-ml-based-hiring-tools-an-engineers-checklist-1kn0)
`dev.to · carreira`

If your company is buying an ML-based hiring tool, there is a decent chance someone will forward the technical due-diligence to an engineer. This is a checklist for that engineer. The business folks will evaluate the demo; your job is to evaluate the model, the data path, and the integration surface, the parts a demo is specifically designed to hide.


  
  
  1. Explainability is an architecture question, not a UI feature


The first thing to establish: can the system produce a per-decision ex

### [Inside OpenAI’s agentic software factory](https://newsletter.pragmaticengineer.com/p/openai-software-factory)
`The Pragmatic Engineer`

A deepdive into how Codex has “taken over” OpenAI, how the frontier lab builds its agentic software factory, and the engineering challenges of one billion users. Details from inside OpenAI

### [8 Remote SWE Jobs You Can Apply to From Anywhere (Sep 16)](https://dev.to/crackr/8-remote-swe-jobs-you-can-apply-to-from-anywhere-sep-16-3n2i)
`dev.to · carreira`

New remote roles just dropped from Mitre Media, Delinea, Pivotal Health, ExtraHop and more. No relocation needed.


  
  
  Tech Lead Full-Stack Rails Engineer


 Mitre Media

Salary: $170k - $200k/yr
Stack: React / JavaScript / Java / Ruby

Find on Crackr.dev





  
  
  Staff Software Development Engineer SDM


 Delinea

Salary: $150k - $185k/yr
Stack: Go / DevOps / Backend

Find on Crackr.dev





  
  
  Senior Software Engineer Case Execution


 Pivotal Health

Salary: $190k - $220k/yr
Sta

## Science & Papers

### [ScienceBuddy: Recursive-in-Recursive Self-Improvement for Interactive Scientific Agents](https://arxiv.org/abs/2609.17523v1)
`arXiv`

We introduce and release ScienceBuddy, an interactive scientific research workspace that brings continually improving scientific agents into researchers' everyday workflows. ScienceBuddy supports researchers in carrying out scientific tasks while transforming their requests, feedback, and execution evidence into tasks and evaluation rubrics for continual learning. At its core is recursive-in-recursive self-improvement, a paradigm that couples harness evolution with model reinforcement learning: 

### [PhysStream: Streaming Physics-Grounded Video Generation with Structured Scene Memory and Fine-Grained Motion Control](https://arxiv.org/abs/2609.17521v1)
`arXiv`

Interactive control for video generation is moving from coarse prompts toward fine-grained, physically meaningful manipulation of dynamic scenes. Yet existing controllable methods either require the full control schedule before generation starts, or use pixel-space signals that dictate object positions rather than physical dynamics. To address these limitations, we propose PhysStream, an autoregressive model for physics-grounded image-to-video synthesis that incorporates structured scene memory-

### [What Breaks Under Pruning in Smart Homes, and When? Evaluating LLM Degradation Across Architectures and Task Complexity](https://arxiv.org/abs/2609.17515v1)
`arXiv`

Pruning can reduce the deployment cost of large language models (LLMs), but its impact on context-grounded tool calling remains poorly understood. We systematically study pruning-induced degradation in smart-home tool calling across four LLMs spanning dense Transformer, dense hybrid, and mixture-of-experts (MoE) architectures, together with depth, width, hybrid, and expert pruning methods. After post-pruning supervised fine-tuning (SFT), we evaluate more than 19,500 instances from three smart-ho

### [ENCP: Episode-Normalized Conformal Prediction for Vision-and-Language Navigation](https://arxiv.org/abs/2609.17499v1)
`arXiv`

Uncertainty estimation for Vision-Language-Navigation (VLN) models is a critical task since it can help identify ambiguous and unreliable predictions, enabling agents to make safer navigation decisions. As one of the most advanced uncertainty estimation frameworks, conformal prediction (CP) offers a promising approach for uncertainty estimation in VLN. However, given that VLN agent requires a sequence of steps, standard calibration in conformal prediction fails to provide coverage guarantee it p

## Trending projects

### [youngyangyang04/llm-master](https://github.com/youngyangyang04/llm-master)
`GitHub`

大模型（LLM）全栈学习路线与中文教程🔥：覆盖 Prompt Engineering、RAG、AI Agent、MCP、微调、模型部署、Transformer、AI 编程与大厂面试，从入门到生产实践。

### [nilbuild/page-mascot](https://github.com/nilbuild/page-mascot)
`GitHub`

A mascot that watches the cursor and blinks when you poke it

## References

1. [Introducing System One Models and Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev) · `Hacker News`
2. [Show HN: An e-ink frame that hears birds and draws them as 1800s illustrations](https://github.com/arnegiacomo/fugleramme) · `Hacker News`
3. [ai-sucks-butt/ai-sucks-butt](https://github.com/ai-sucks-butt/ai-sucks-butt) · `GitHub`
4. [A single firm is behind OpenAI, Anthropic, and Meta hacking scandals](https://www.effort.news/irregular) · `Hacker News`
5. [ukisai/Swift-Qwen3.8-27b](https://huggingface.co/ukisai/Swift-Qwen3.8-27b) · `Hugging Face`
6. [The Download: AI doomers, whistleblowing agents, and de-aged livers](https://www.technologyreview.com/2026/09/15/1144141/the-download-ai-extinction-whistleblowing-agents-donated-livers/) · `MIT Tech Review`
7. [AI models need more data about biology, and OpenAI is paying to create it](https://www.technologyreview.com/2026/09/15/1144129/ai-models-need-more-data-about-biology-and-openai-is-paying-to-create-it/) · `MIT Tech Review`
8. [25 years of mass surveillance is enough](https://www.schneier.com/blog/archives/2026/09/25-years-of-mass-surveillance-is-enough.html) · `Hacker News`
9. [An update on Wayback Machine access](https://blog.archive.org/2026/09/15/an-update-on-wayback-machine-access/) · `Hacker News`
10. [Dario, Please](https://pop.rdi.sh/dario-please/) · `Hacker News`
11. [Robots are waiting for a ChatGPT moment: Nvidia’s Les Karpas explains why at TechCrunch Disrupt 2026](https://techcrunch.com/2026/09/16/robots-are-waiting-for-a-chatgpt-moment-nvidias-les-karpas-explains-why-at-techcrunch-disrupt-2026/) · `TechCrunch`
12. [These Companies Want Interns This Summer (Sep 16)](https://dev.to/crackr/these-companies-want-interns-this-summer-sep-16-3a1o) · `dev.to · carreira`
13. [Evaluating ML-Based Hiring Tools: An Engineer's Checklist](https://dev.to/john_zacharia/evaluating-ml-based-hiring-tools-an-engineers-checklist-1kn0) · `dev.to · carreira`
14. [Inside OpenAI’s agentic software factory](https://newsletter.pragmaticengineer.com/p/openai-software-factory) · `The Pragmatic Engineer`
15. [8 Remote SWE Jobs You Can Apply to From Anywhere (Sep 16)](https://dev.to/crackr/8-remote-swe-jobs-you-can-apply-to-from-anywhere-sep-16-3n2i) · `dev.to · carreira`
16. [ScienceBuddy: Recursive-in-Recursive Self-Improvement for Interactive Scientific Agents](https://arxiv.org/abs/2609.17523v1) · `arXiv`
17. [PhysStream: Streaming Physics-Grounded Video Generation with Structured Scene Memory and Fine-Grained Motion Control](https://arxiv.org/abs/2609.17521v1) · `arXiv`
18. [What Breaks Under Pruning in Smart Homes, and When? Evaluating LLM Degradation Across Architectures and Task Complexity](https://arxiv.org/abs/2609.17515v1) · `arXiv`
19. [ENCP: Episode-Normalized Conformal Prediction for Vision-and-Language Navigation](https://arxiv.org/abs/2609.17499v1) · `arXiv`
20. [youngyangyang04/llm-master](https://github.com/youngyangyang04/llm-master) · `GitHub`
21. [nilbuild/page-mascot](https://github.com/nilbuild/page-mascot) · `GitHub`

---

Edition generated automatically by stdout · redação Groq (llama-3.3-70b-versatile).